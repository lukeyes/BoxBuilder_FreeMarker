...
public class ArtieBox2 extends FritaBox {
	...
	private List<Polygon> generateSideA(
			float originX,
			float originY,
			float boxWidth, 
			float boxHeight, 
		...
		drawPanelSection(originX, originY, boxWidth, boxHeight, openWidth,
				openHeight, nutWidth, screwDiameter, drawNutTrap, polygonList);
	...
	private List<Polygon> generateSideC(
	...
	private void drawPanelSection(float originX, float originY, float boxWidth,
			float boxHeight, float openWidth, float openHeight, float nutWidth,
			float screwDiameter, boolean drawNutTrap, List<Polygon> polygonList) {
		// find center
		...
		if( drawNutTrap ) {
			...
			polygonList.add(nutTrapTopLeft);
			...
			polygonList.add(nutTrapTopRigth);
			...
			polygonList.add(nutTrapBottomLeft);

			Polygon nutTrapBottomRight = Section.drawSquare(originX+tRX, originY+blY, nutWidth, nutWidth);
			polygonList.add(nutTrapBottomRight);			
		} else {

			float screwMargin = (nutWidth - screwDiameter)/2;

			float tlX = openX - (nutWidth+screwDiameter+screwMargin);
			float tlY = openY - (nutWidth+screwDiameter+screwMargin);

			float tRX = centerX + (openWidth/2) + nutWidth + screwMargin;
			float blY = centerY + (openHeight/2) + nutWidth + screwMargin;

			Polygon screwHoleTopLeft = Section.drawSquare(originX + tlX, originY + tlY, screwDiameter, screwDiameter);
			polygonList.add(screwHoleTopLeft);

			Polygon screwHoleTopRight = Section.drawSquare(originX + tRX, originY + tlY, screwDiameter, screwDiameter);
			polygonList.add(screwHoleTopRight);

			Polygon screwHoleBottom = Section.drawSquare(originX + tlX, originY + blY, screwDiameter, screwDiameter);
			polygonList.add(screwHoleBottom);

			Polygon screwHoleBottomRight = Section.drawSquare(originX + tRX, originY + blY , screwDiameter, screwDiameter);
			polygonList.add(screwHoleBottomRight);
